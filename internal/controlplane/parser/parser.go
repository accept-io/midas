...
func ParseYAML(data []byte) (ParsedDocument, error) {
	...
	case types.KindBusinessServiceCapability:
		var doc types.BusinessServiceCapabilityDocument
		if err := strictUnmarshal(data, &doc); err != nil {
			return ParsedDocument{}, fmt.Errorf("failed to parse BusinessServiceCapability document: %w", err)
		}
		return wrapDocument(doc), nil
	...
	case types.KindGovernanceExpectation:
		...
		if err := strictUnmarshal(data, &doc); err != nil {
			return ParsedDocument{}, fmt.Errorf("failed to parse GovernanceExpectation document: %w", err)
		}
		return wrapDocument(doc), nil

	case types.KindAISystem:
		var doc types.AISystemDocument
		if err := strictUnmarshal(data, &doc); err != nil {
			return ParsedDocument{}, fmt.Errorf("failed to parse AISystem document: %w", err)
		}
		return wrapDocument(doc), nil

	case types.KindAISystemVersion:
		var doc types.AISystemVersionDocument
		if err := strictUnmarshal(data, &doc); err != nil {
			return ParsedDocument{}, fmt.Errorf("failed to parse AISystemVersion document: %w", err)
		}
		return wrapDocument(doc), nil

	case types.KindAISystemBinding:
		var doc types.AISystemBindingDocument
		if err := strictUnmarshal(data, &doc); err != nil {
			return ParsedDocument{}, fmt.Errorf("failed to parse AISystemBinding document: %w", err)
		}
		return wrapDocument(doc), nil
	...
	case types.KindDriftDefinition:
		var doc types.DriftDefinitionDocument
		if err := strictUnmarshal(data, &doc); err != nil {
			return ParsedDocument{}, fmt.Errorf("failed to parse DriftDefinition document: %w", err)
		}
		return wrapDocument(doc), nil

	default:
		return ParsedDocument{}, fmt.Errorf("unsupported kind: %q (must be Surface, Agent, Profile, Grant, Capability, Process, BusinessService, BusinessServiceCapability, BusinessServiceRelationship, GovernanceExpectation, AISystem, AISystemVersion, AISystemBinding, FailModePolicy, or DriftDefinition)", meta.Kind)
	}
}
